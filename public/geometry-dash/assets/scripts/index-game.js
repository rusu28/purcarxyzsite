...
preload() {
    ...
    this.load.image("purcar_head", "/assets/snake/head.png");
    this.load.image("purcar_2", "/assets/purcar2.jpeg");
    this.load.image("purcar_3", "/assets/purcar3.jpeg");
    this.load.image("purcar_4", "/assets/purcar4.jpeg");
    this.load.image("purcar_5", "/assets/purcar5.jpeg");
    this.load.image("purcar_6", "/assets/purcar6.jpeg");
...
class ps {
  ...
  _createSprites() {
    ...
    const selectedPurcarAvatar = window.purcarDashSelectedAvatar || "purcar_head";
...
class xs extends Phaser.Scene {
  ...
  create() {
    ...
    this._copyrightText = this.add.text(0, 625, "© 2026 Purcar Dash", {
      fontSize: "14px",
      color: "#ffffff",
      fontFamily: "Arial"
...
"ship_71_001.png", "ship_72_001.png", "ship_73_001.png", "ship_74_001.png", "ship_75_001.png", "ship_76_001.png", "ship_77_001.png", "ship_78_001.png", "ship_79_001.png"
    ...
    this._openIconSelector = (startTab = "icon") => {
      ...
      const titleTxt = this.add.bitmapText(sw / 2, 60, "goldFont", "Purcar Characters", 32)
    ...
    topBar.strokePath();
      ...
      const _getPreviewFrame = (tab) => {
        if (tab === "icon") {
          return window.purcarDashSelectedAvatar || "purcar_head";
        }
      ...
      const _getMaxPages = (tab) => {
        const list = tab === "icon" ? (window.purcarDashAvatarKeys || ["purcar_head"]) : _iconFrameSets[tab];
        return Math.ceil(list.length / iconsPerPage);
      };
      const _prevPage = () => {
      ...
      const _buildGrid = (tab, page = 0) => {
        ...
        const allFrames = tab === "icon" ? (window.purcarDashAvatarKeys || ["purcar_head"]) : _iconFrameSets[tab];
        ...
        frames.forEach((frame, idx) => {
          ...
          if ((tab === "icon" && frame === (window.purcarDashSelectedAvatar || "purcar_head")) || (tab !== "icon" && frame.replace("_001.png", "") === window[prop])) {
          ...
          ((capturedFrame, capturedImg, capturedExtra, capturedOrigScale) => {
            ...
            hitRect.on("pointerup",    () => {
              ...
              if (tab === "icon") {
                window.purcarDashSelectedAvatar = capturedFrame;
                localStorage.setItem("purcarDashSelectedAvatar", capturedFrame);
                _refreshPreview("icon", capturedFrame);
...
this._escKey.on("down", () => {
  ...
  _buildPauseOverlay() {
    ...
    this._noclipCheckbox = this._createPauseToggleButton(this._pauseContainer, textureY - 300, 570, "Noclip", window.noClip, value => {
      window.noClip = value;
    });

    this._showHitboxesCheckbox = this._createPauseToggleButton(this._pauseContainer, textureY - 100, 570, "Hitbox", window.showHitboxes, value => {
      window.showHitboxes = value;
      this._player.setShowHitboxes(value);
    });

    this._showPercentageCheckbox = this._createPauseToggleButton(this._pauseContainer, textureY + 100, 570, "%", window.showPercentage, value => {
      window.showPercentage = value;
      this._percentageLabel.setVisible(value);
    });

    this._solidWaveCheckbox = this._createPauseToggleButton(this._pauseContainer, textureY + 250, 570, "Solid", window.solidWave, value => {
  ...
  _buildUpdateLogPopup() {
    ...
    */
    const updateEntries = [
      ...
      { text: "I love this menu :)", scale: 0.65, color: 0xaaddff },
