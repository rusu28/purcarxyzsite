...
import { createSaveScreen } from "./saveGame.js"
import { cargarSonido } from "./cargarSonido.js"
import { Character } from "./characters.js"
import {config} from "./config.js"
import {dictionary} from "./dictionary.js"
import Background from "./backgrounds.js"
import {managePoem} from "./managePoem.js"
import remove from "./gameStart.js"
...
const hoverSound = cargarSonido("/dokisecret/api/sound/sfx/hover-sound");
const selectSound = cargarSonido("/dokisecret/api/sound/sfx/select");
const slap = cargarSonido("/dokisecret/api/sound/sfx/select")
...
if(chapter === "the-end" || localStorage.getItem("the-end")){
    ...
    window.open("/dokisecret/happy.html", "_self")
}

const dataRoute = route => {
    if (!route || route === "undefined") return "/dokisecret/data/start.json";
    return `/dokisecret/data/${String(route).replace(/^\//, "").replace(/\//g, "-")}.json`;
}

//init
fetch(dataRoute(chapter))
    .then (res => res.json())
    .then (async x => {
        // acÃ¡ arranca el programa. antes de que se empiece a ejecutar la funciÃ³n que recorre el array al que se le hizo fetch,
        // el programa observa el primer elemento en busca de configuraciones globales
        next = x[0]?.next;
        if( x[0].usesRoute) {
            console.log(route)
            if(!route) window.open("/dokisecret/poem.html", "_self")
            else{
                let fetchCall = ""
                ...
                const res = await fetch(dataRoute(fetchCall))
...
if (song) music = cargarSonido("/dokisecret/api/sound/music/" + song)
...
function manageOptions(optionObj){
    ...
    //la opcion aca es "si estas siguiendo la ruta de tal", independientemente de si le gusto o amÃ³ el poema
...
function manageMusic(musicSrc) {
    ...
    if(musicSrc !=="stop"){
        music = cargarSonido("/dokisecret/api/sound/music/" + musicSrc, true);
        song = musicSrc
...
function manageMusicGroup(group){
    ...
    if (group.new){
        ...
        audioN = cargarSonido("/dokisecret/api/sound/music/" + group.nat, true)
        audioM = cargarSonido("/dokisecret/api/sound/music/" + group.mon, true)
        audioS = cargarSonido("/dokisecret/api/sound/music/" + group.say, true)
        audioY = cargarSonido("/dokisecret/api/sound/music/" + group.yu, true)
        audioNeutral = cargarSonido("/dokisecret/api/sound/music/" + group.neutral, true)
        myFeelings = cargarSonido("/dokisecret/api/sound/music/my-feelings.mp3")
...
function saveGame(option) {
    ...
    document.getElementById("menu-btn").addEventListener("click", () => {window.open("/dokisecret/index.html", "_self") })
...
const openMenu = () => {
    window.open("/dokisecret/index.html", "_self")
}
